...
use crate::api::{
    ...
    code::{CodeGetStatsParams, CodeSearchParams, CodeSearchService},
    docs::{DocsGetStatsParams, DocsSearchParams, DocsSearchService},
    groups::{
...
impl KeycloakToolHandler {
    ...
    #[tool(
        description = "Search Keycloak documentation using semantic search. Returns relevant documentation sections based on natural language queries."
    )]
    async fn docs_search(
        ...
        use crate::vector::{EmbeddingService, MilvusClient};

        let config =
            Config::from_env().map_err(|e| McpError::internal_error(e.to_string(), None))?;
    ...
    #[tool(
        description = "Get statistics about the indexed Keycloak documentation including total vectors and last index time."
    )]
    ...
    #[tool(
        description = "Search Keycloak source code using semantic search. Find implementations, classes, methods based on natural language queries. Supports filtering by language, file path, and chunk type (function, class, method)."
    )]
    ...
    #[tool(
        description = "Get statistics about the indexed Keycloak source code including total vectors and last index time."
    )]
